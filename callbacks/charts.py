...
def register_charts_callbacks(app: Dash, all_sentinel: str) -> None:
    """
    Register three independent callbacks so that:
      - Map listens only to global filters (data/active + value/time filters)
      - Bar listens to its own x/y + global filters
      - Pie listens to its own column + global filters
    ...
    def _render_map(data_json, active_cols, filter_col, filter_val, time_col, year_values):
        ...
        return _build_map(df, hover_col=time_col, color_col=map_color_col)
    ...
    def _render_bar(data_json, active_cols, filter_col, filter_val, time_col, year_values, x_col, y_col):
        ...
        df = json_to_df(data_json)
        df = _subset_to_active(df, active_cols, also_keep=[time_col, x_col, y_col])
        if df.empty:
            return empty

        df = _apply_value_filter(df, filter_col, filter_val, all_token=all_sentinel)
        df = _apply_year_filter(df, time_col, year_values)
        return _build_bar(df, x_col, y_col)
    ...
    def _render_pie(data_json, active_cols, filter_col, filter_val, time_col, year_values, pie_col):
        ...
        df = json_to_df(data_json)
        df = _subset_to_active(df, active_cols, also_keep=[time_col, pie_col])
        if df.empty:
            return empty

        df = _apply_value_filter(df, filter_col, filter_val, all_token=all_sentinel)
        df = _apply_year_filter(df, time_col, year_values)
        return _build_pie(df, pie_col)
